...
<div class="item-form">
    ...
    <div class="row">
        <?php foreach($characteristic AS $chara){ $value = null; ?>
        ...
        <div class="col-sm-3 col-xs-12">
            <div class="form-group field-item-warranty">
                ...
                <?php }elseif($chara->type == 'checkbox'){ ?>
                    <label><input type="checkbox" id="ItemCharacteristic<?=$chara->id?>" name="ItemCharacteristic[<?=$chara->id?>]" value="1"> <?=$chara->name?> (<?=$chara->dimension?>)</label>
                 <?php }else{ ?>
                    <label class="control-label" for="ItemCharacteristic<?=$chara->id?>"><?=$chara->name?> (<?=$chara->dimension?>)</label>
